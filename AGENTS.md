...
- Commands (run from repo root):
  - Test: `python3 -m pytest tests/` (25 tests; harness in `tests/test_lattice.py` shells out to `./lattice` per `tests/tests.json` case).
...
- Runnable programs live in `tests/`. Files in `examples/` and `spec.lattice` are illustrative spec/demo programs and are not part of the test suite (not guaranteed to compile).
